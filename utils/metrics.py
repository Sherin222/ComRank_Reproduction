...
def PrecisionAtK(pred_scores, target_labels, K):
    '''
    Computing Precision at K
    ...
    target_labels : Tensor
        MxQ Tensor storing the real labels, if the ith instance belongs to the
        jth class, then target_labels[i,j] equals to +1, otherwise
        target_labels[i,j] equals to 0.
    K : int
        The number of top elements to look at for computing precision.

    Returns
    -------
    precision_at_k : float
        The average precision at K over all instances.
    '''
    _, top_k_indices = torch.topk(pred_scores, K, dim=1)
    relevant = torch.gather(target_labels, 1, top_k_indices)

    # Count how many of the top K predictions are true positives per instance
    correct_preds = torch.sum(relevant == 1, dim=1).float()

    # Precision at K is the average of these counts divided by K
    precision_at_k = torch.mean(correct_preds / K).item()
